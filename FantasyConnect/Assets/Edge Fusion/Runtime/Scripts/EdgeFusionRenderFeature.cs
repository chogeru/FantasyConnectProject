...
#else
            // Pre-RenderGraph URP copies _CameraDepthTexture after the skybox, so run once the copy exists
            renderPass.renderPassEvent = renderPassEvent < RenderPassEvent.AfterRenderingSkybox + 1 ? RenderPassEvent.AfterRenderingSkybox + 1 : renderPassEvent;
#endif
