Shader "Hidden/lilToonGem"
{
    Properties
    {
        ...
        //----------------------------------------------------------------------------------------------------------------------
        // Refraction
                        _RefractionStrength         ("", Range(-1,1)) = 0.5
        _RefractionFresnelPower   ("", Range(0.01, 10)) = 1.0

        //----------------------------------------------------------------------------------------------------------------------
        //Gem
                        _GemChromaticAberration     ("", Range(0, 1)) = 0.02
                        _GemEnvContrast             ("", Float) = 2.0
                _GemEnvColor                ("", Color) = (1,1,1,1)
                        _GemParticleLoop            ("", Float) = 8
                _GemParticleColor           ("", Color) = (4,4,4,1)
                        _GemVRParallaxStrength      ("", Range(0, 1)) = 1
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Assets/開発用アセット/シェーダー/lilToon/Shader/Includes/lil_common_input_opt.hlsl"
